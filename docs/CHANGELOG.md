...
## May 30, 2026 — Release hardening: assets, AI chat, CI gate, and EXC_BAD_ACCESS crash fix
...
- Updated `DroppedItemActor::isAlive()` in `include/actor_dropped_item.h` to return `false` if `m_item.data == nullptr`. This ensures that defunct actors whose close buttons have been clicked are automatically deleted and removed from the active actors list by `ActorManager::cleanup()`, also resolving a memory leak.
...
- **Fixed a secondary crash due to uninitialized config fields in rendering tests.** Under the GTest runner, the global configuration structures are not automatically initialized from files. This caused mathematical helper calls (like vector normalization) inside goose rendering routines to consume uninitialized physics values, producing `NAN` coordinate values. When these `NAN` values reached `CGContextDrawPath`, they caused a CoreGraphics internal segmentation fault (in `ripc_GetColor`) on remote VM software rasterizers during the `GooseRender.DrawGoose_FacingRight` test. Resolved by explicitly initializing `isoScaleX` and `isoScaleY` to `1.0f` in the test helper `createTestGoose()` and in the `GooseRender.DrawThreeGeese_AllVisible` test in `tests/platform/macos/test_goose_rendering.mm`.
- **Eliminated deprecation compiler warnings on macOS.** Removed the deprecated `setOneShot:` call from `initWithDrawBlock:deviceX:deviceY:width:height:` inside `src/platform/macos/behavior_element_window.mm` to maintain a completely clean, 0-warning compilation pipeline.
