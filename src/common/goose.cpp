...
Goose::Goose(int id_, const std::string& name_, int screenW, int screenH)
    : id(id_), name(name_) {
    pos = {(float)(rand() % (int)(screenW - g_config.spawn.marginX * 2) + g_config.spawn.marginX),
           (float)(rand() % (int)(screenH - g_config.spawn.marginY * 2) + g_config.spawn.marginY)};
    target = pos;
...
void Goose::UpdateRig() {
    ...
    int targetState = (currentSpeed >= g_config.rig.runSpeedThreshold) ? 1 : 0;
    rig.neckLerp = Lerp(rig.neckLerp, (float)targetState, g_config.rig.neckLerpRate);

    float neckH = Lerp(g_config.rig.neckHeightIdle, g_config.rig.neckHeightMoving, rig.neckLerp);
    float neckExt = Lerp(g_config.rig.neckExtIdle, g_config.rig.neckExtMoving, rig.neckLerp);

    rig.neckBase = rig.body + fwd * g_config.rig.neckBaseX + up * (g_config.rig.footOffsetY * back);
    rig.neckHead = rig.neckBase + fwd * (neckExt + g_config.rig.headForwardBias * back) + up * neckH;
...
void Goose::SolveFeet(double time) {
    ...
    float denom = std::max(1.0f, (g_config.movement.baseRunSpeed - g_config.movement.baseWalkSpeed));
    float speed01 = Clamp((speed - g_config.movement.baseWalkSpeed) / denom, 0.0f, 1.0f);
    float stepTrigger = Lerp(g_config.step.stepTriggerWalk, g_config.step.stepTriggerRun, speed01);
    float overshoot = Lerp(g_config.step.overshootWalk, g_config.step.overshootRun, speed01);
    float baseDur = Lerp(g_config.step.durationWalk, g_config.step.durationRun, speed01);
    ...
    auto UpdateFoot = [&](FootState& f, Vector2 home) {
        if (f.moveStartTime < 0) {
            ...
            if (dist > stepTrigger) {
                ...
                float distFactor = Clamp(dist / g_config.step.distFactorBase, g_config.step.distFactorMin, g_config.step.distFactorMax);
                f.moveDuration = Clamp(baseDur * distFactor, g_config.step.durationMin, g_config.step.durationMax);
            }
        } else {
            Vector2 target = home + f.moveDir * overshoot;
            float p = (float)(time - f.moveStartTime) / std::max(g_config.step.minDuration, f.moveDuration);
            ...
            if (p >= 1.0f) {
                ...
                if (mudEnabled && (rand() % 100) < mudChance) {
                    Footprint fp;
                    fp.pos = home;
                    fp.dir = dir + ((&f == &rig.lFoot) ? g_config.step.leftFootAngle : g_config.step.rightFootAngle);
                    fp.timeSpawned = time;
...
CursorAction Goose::Update(double dt, double time, int w, int h, const CursorState& cursor) {
    ...
    float tSpeed = (dist > g_config.movement.runDistanceThreshold || state == FETCHING || state == CHASE_CURSOR || state == SNATCH_CURSOR || state == RETURNING)
    ...
    if (Vector2::Length(vel) > g_config.physics.curveFadeMinVel) {
        ...
        steerForce += tangent * (parabolicCurvature * currentSpeed * g_config.physics.curveTangentForce * curveFade);
    }
    ...
    if (state == WANDER || state == FETCHING) {
        for (auto& other : g_geese) {
            ...
            if (d > g_config.spawn.separationMinDistance && d < g_config.spawn.separationMaxDistance) {
                float strength = (g_config.spawn.separationMaxDistance - d) / g_config.spawn.separationMaxDistance;
                Vector2 away = Vector2::Normalize(pos - other.pos);
                steerForce += away * (strength * g_config.movement.maxForce * g_config.spawn.separationForceMultiplier);
            }
        }
    }

    if (state != FETCHING) {
        float lookAhead = currentSpeed * g_config.physics.edgeLookAheadSpeed + g_config.physics.edgeLookAheadBase;
        ...
        if (!g_config.cursor.multiMonitorEnabled && !g_monitors.empty()) {
            for (auto& m : g_monitors) {
                if (m.x == 0 && m.y == 0) {
                    bMaxX = (float)m.width; bMaxY = (float)m.height; break;
                }
            }
        }
...
void Goose::StartFetch(int w, int h) {
    ...
    switch (side) {
        case 0: target = {-edgeMargin, (float)(rand() % h)}; break;
        case 1: target = {(float)w + edgeMargin, (float)(rand() % h)}; break;
        case 2: target = {(float)(rand() % w), -edgeMargin}; break;
        case 3: target = {(float)(rand() % w), (float)h + edgeMargin}; break;
    }
