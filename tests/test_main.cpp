...
// and those tests fail. Requires the working directory to contain Assets/
...
int main(int argc, char **argv) {
    // FIRST, before anything that might resolve a path — including
    // CrashLogger_Init(), which redirects stderr into ConfigDirPath()/logs and
    // used to create config/logs in the repo when run from the source tree.
    // ConfigDirPath() falls back to <cwd>/config when it finds a config.toml
    // there, which is exactly what the repo root looks like and exactly where
    // ctest runs. The suite never loads that file, so a save from unsandboxed
    // state rewrites it with registry defaults — the mcp_port 31073->31072 flip.
    setenv("CADGOOSE_CONFIG_DIR", "/tmp/cadgoose-test-config", 1);
    setenv("CADGOOSE_HEADLESS_TEST", "1", 1);

    // ...and PROVE it took, rather than trusting that no static initializer
    // resolved a path first (initialization order across translation units is
    // unspecified). Refusing to run is the right failure: a suite that
    // silently corrupts real state is worse than one that does not start.
    {
        const std::filesystem::path dir = ConfigDirPath();
        const std::filesystem::path repo = std::filesystem::current_path() / "config";
        std::error_code ec;
        if (std::filesystem::equivalent(dir, repo, ec)) {
            fprintf(stderr,
                    "FATAL: the test config dir resolved to the repo's own "
                    "config/ (%s).\nRunning would overwrite real state. "
                    "Something resolved a path before main().\n",
                    dir.string().c_str());
            return 2;
        }
    }
...
// ---------------------------------------------------------------------------
// Config isolation. main() sandboxes CADGOOSE_CONFIG_DIR before any path can
// resolve; this asserts the same property from inside a test, so the guarantee
// survives someone rewriting main — or a test unsetting the var mid-suite
// (ConfigPath.EnvVarOverridesDefault once did exactly that, silently).
// ---------------------------------------------------------------------------
TEST(ConfigIsolation, TestsNeverWriteIntoTheRepoConfigDir) {
    const std::filesystem::path dir = ConfigDirPath();
    const std::filesystem::path repo = std::filesystem::current_path() / "config";
    std::error_code ec;
    EXPECT_FALSE(std::filesystem::equivalent(dir, repo, ec))
        << "config dir is " << dir << " -- the suite would overwrite real state";
}
